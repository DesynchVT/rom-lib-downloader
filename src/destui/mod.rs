...
use crate::dessh::{self, create_ssh_session};
use crate::utils;
mod render;
    ...
    fn download(&mut self) {
        ...
        thread::spawn(move || {
            ...
            for rom_item in roms {
                ...
                utils::unzip(&part_path, &completed_download);
    ...
    fn set_mode(&mut self, new_mode: AppMode) {
        match new_mode {
            ...
            AppMode::SelectRom => {
                ...
                let existing_roms = dessh::list_roms(self.sess, &self.current_console);
...
fn app(
    ...
    while !app_state.exit {
        terminal.draw(|frame| render::render(frame, app_state))?;
